...
- **Dashboard Admin (autentikasi & role)**
  - Hanya akun terdaftar dengan role `admin` yang bisa login ke dashboard (menggunakan Supabase Auth); akun admin di-seed langsung ke database, tidak ada pendaftaran publik.
  - Admin dapat mengelola pesanan, mengelola produk beserta pengaturan margin harga per produk, dan melihat statistik (pendapatan, jumlah pesanan, produk terlaris).

- **Bot Discord (kanal pembelian alternatif)**
  - Pelanggan dapat menjelajah katalog dan membeli produk langsung dari Discord melalui bot (slash command + tombol), tanpa membuat akun (guest) — identitas pembeli cukup disimpan sebagai `discord_user_id`.
  - Bot membuat pesanan di backend yang sama, lalu menampilkan **kode QRIS Midtrans langsung di Discord** (bukan dikirim ke DM).
  - Setelah pembayaran terkonfirmasi webhook Midtrans, sistem otomatis memesan ke Premiumku dan **mengirim detail produk digital ke pembeli melalui DM Discord**.
  - Semua produk bersifat auto-deliver dari supplier (Premiumku), sehingga tidak ada pengiriman manual di kanal Discord.

### Kebutuhan Non-Fungsional

- **Keamanan:**
  - `api_key` Premiumku dan server key Midtrans hanya disimpan di sisi server, tidak pernah muncul di frontend.
  - `DISCORD_BOT_TOKEN` hanya disimpan di environment variable service bot, tidak pernah diekspos ke frontend.
...
### Fase 4 — Dashboard Admin & Autentikasi

Back-office khusus admin toko untuk mengelola pesanan, produk (termasuk margin harga), dan memantau statistik. Area ini tidak terbuka untuk umum dan hanya dapat diakses oleh akun terdaftar yang berperan `admin`.

- **Login Admin & Kontrol Akses (RBAC)**
  Login hanya untuk akun yang terdaftar di sistem. Autentikasi memakai Supabase Auth; akun admin di-seed langsung ke database, tidak ada pendaftaran publik. Semua endpoint/halaman dashboard dilindungi dan hanya user ber-role `admin` yang boleh mengakses; user lain (customer) ditolak/dialihkan.

- **Kelola Pesanan**
  Admin dapat melihat seluruh pesanan pelanggan, memeriksa detail (produk, jumlah, total, kontak pembeli, status pembayaran & pengiriman), serta memperbarui status bila diperlukan untuk penanganan manual.

- **Kelola Produk & Margin Harga**
  Admin dapat menambah, menyunting, dan menonaktifkan produk yang tampil di katalog. Untuk setiap produk, admin dapat mengatur **margin harga** berupa markup **tetap dalam Rupiah (flat)** di atas harga dasar dari Premiumku. Margin selalu bertipe flat — bukan persen. Contoh: modal 500 ditambah margin 1.500 menghasilkan harga jual 2.000. Harga jual yang tampil ke pelanggan dihitung dari `harga_dasar + margin`. Perubahan margin hanya berlaku untuk pesanan berikutnya karena `order_items` menyimpan snapshot harga saat pembelian.

- **Statistik**
  Halaman ringkasan yang menampilkan total pendapatan, jumlah pesanan berdasarkan status, produk terlaris, dan tren penjualan untuk memantau performa toko.

### Fase 5 — Bot Discord

Kanal pembelian alternatif selain web, memakai backend, database, dan integrasi QRIS/Premiumku yang sama. Bot dibangun sebagai service Node.js terpisah (`discord.js`) yang memanggil REST API backend — bukan mengakses database langsung.

- **Katalog via Discord**
  User menjalankan slash command (mis. `/produk`) untuk menampilkan daftar produk dalam bentuk embed, lengkap dengan kategori, harga, dan status stok. Navigasi memakai tombol paging atau dropdown kategori.

- **Detail & Beli**
  User memilih produk, lalu bot menampilkan detail produk beserta tombol **Beli**. Karena pembeli bersifat guest, identitas cukup diambil dari `discord_user_id` (tanpa login/registrasi).

- **Checkout & QRIS di Discord**
  Saat tombol Beli ditekan, backend membuat pesanan berstatus **menunggu pembayaran** dan membuat transaksi QRIS Midtrans. Bot menampilkan **gambar kode QRIS langsung di Discord** (balasan interaksi/ephemeral), bukan melalui DM, disertai nominal dan batas waktu pembayaran.

- **Auto-proses & Kirim via DM**
  Setelah webhook Midtrans mengonfirmasi pembayaran sukses, backend otomatis memanggil Create Order Premiumku dengan `ref_id` unik. Begitu produk siap, bot mengirimkan detail produk digital ke **DM Discord** pembeli.

- **Cek Status Pesanan**
  User dapat memeriksa status pesanan miliknya lewat slash command (mis. `/pesanan-saya`), yang hanya menampilkan pesanan yang terhubung dengan `discord_user_id` miliknya agar tidak membocorkan data pembeli lain.
...
### Alur Dashboard Admin (Fase 4)

17. Admin membuka halaman login dashboard dan masuk dengan kredensial Supabase Auth.
18. Sistem memverifikasi keanggotaan/role; hanya user ber-role `admin` yang diizinkan, selainnya ditolak.
19. Admin masuk ke dashboard dan melihat statistik ringkas (pendapatan, jumlah pesanan, produk terlaris).
20. Admin membuka daftar pesanan untuk memantau dan memperbarui status, atau membuka halaman produk untuk menyunting produk dan mengatur margin harga.

### Alur Pembelian via Bot Discord (Fase 5)

21. User menjalankan slash command di Discord (mis. `/produk`) dan memilih produk dari katalog yang ditampilkan bot.
22. Bot menampilkan detail produk dan tombol **Beli**.
23. User menekan tombol Beli. Backend membuat pesanan berstatus **menunggu pembayaran** dan membuat transaksi QRIS ke Midtrans Core API.
24. Bot menampilkan **gambar QRIS langsung di Discord** (bukan DM) beserta nominal dan batas waktu.
25. User memindai QRIS dan menyelesaikan pembayaran; Midtrans mengirim webhook sukses ke backend.
26. Backend memverifikasi webhook, mengubah status pesanan menjadi **lunas**, lalu otomatis memanggil Create Order Premiumku dengan `ref_id` unik.
27. Setelah produk siap, bot mengirimkan detail produk digital ke **DM Discord** pembeli, dan pesanan ditandai **selesai**.
28. User dapat mengecek status pesanannya kapan saja lewat slash command, yang hanya menampilkan pesanan milik `discord_user_id`-nya.
...
7. **Supabase Auth** — autentikasi dan kontrol role (`admin`/`customer`); akun admin di-seed langsung ke database. Juga menyediakan PostgreSQL terkelola sebagai database produksi aplikasi.
8. **Dashboard Admin (React)** — halaman back-office `/admin` yang seluruh route-nya dilindungi berdasarkan role admin.
9. **Bot Discord (Node.js + discord.js)** — service terpisah yang menyediakan kanal pembelian alternatif. Bot **memanggil REST API backend** (bukan langsung ke database) sehingga logika pesanan, QRIS, dan Premiumku tetap satu pintu di backend. Proses bot harus berjalan persisten (long-running), bukan serverless.

> Catatan arsitektur: Database produksi memakai **PostgreSQL terkelola Supabase** (menggantikan Postgres lokal/self-hosted), sementara Drizzle ORM tetap dipakai untuk mendefinisikan skema dan query. Alur interaksi backend ↔ database pada diagram tetap berlaku.
...
| `discord_user_id` | varchar | ID user Discord pembeli (untuk pembelian via bot); nullable bila pembelian dari web |
...
### Tambahan Schema untuk Dashboard Admin & Autentikasi

`users` — akun yang bisa masuk ke sistem. Dipakai terutama untuk role admin (customer tidak wajib punya akun; pembayaran pelanggan tetap berjalan sebagai guest). Data kredensial & sesi dikelola Supabase Auth.

| Kolom | Tipe | Kegunaan |
|---|---|---|
| `id` | uuid (PK) | Identitas unik, diselaraskan dengan user id Supabase Auth |
| `email` | varchar (unik) | Email login |
| `role` | varchar | Peran akses: `admin` (bisa ke dashboard) / `customer` |
| `full_name` | varchar | Nama lengkap user |
| `is_active` | boolean | Menandai akun aktif |
| `created_at` | timestamp | Waktu akun dibuat |

Perubahan pada tabel `products` (dukungan margin harga):

| Kolom | Tipe | Kegunaan |
|---|---|---|
| `provider_price` | integer | Harga dasar produk dari Premiumku (sebelum margin) |
| `margin_value` | integer | Margin tetap (flat) dalam Rupiah yang diatur admin; selalu bertipe flat, bukan persen |

- Harga jual yang tampil dihitung: `harga_jual = provider_price + margin_value` (contoh: provider 500 + margin 1.500 = jual 2.000). Kolom `price` yang sudah ada dapat berisi hasil akhir tersebut atau dihitung saat query.
- `order_items.price` tetap snapshot harga saat transaksi, jadi perubahan margin oleh admin tidak mengubah pesanan lama.
...
| **Database** | PostgreSQL (Supabase) | Menyimpan data produk, pesanan, pembayaran, pengiriman produk digital, dan user/role. Pakai database terkelola Supabase |
...
| **Auth & Role** | Supabase Auth | Autentikasi login admin dan kontrol role (`admin`/`customer`); akun admin di-seed langsung ke database, tanpa registrasi publik |
...
| **Bot Discord** | Node.js + discord.js | Service terpisah untuk kanal pembelian via Discord (slash command + tombol); memanggil REST API backend, menampilkan QRIS di Discord, dan mengirim produk digital via DM. Berjalan persisten dengan `DISCORD_BOT_TOKEN` di environment variable |
